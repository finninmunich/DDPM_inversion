...
if __name__ == "__main__":
    ...
    # model_id = "/home/turing/cfs_cz/finn/codes/image-edition/examples/text_to_image/stable-diffusion-v1-5"
    model_id = "/home/turing/cfs_cz/finn/codes/image-edition/examples/text_to_image/experiments/full_training/jidu-III-sunnyday2snowyday-v2/bs_6_lr_1e6_4gpu"
    ...
    for i in range(len(full_data)):
        ...
        if args.mode == "p2pddim" or args.mode == "ddim":
            ...
            _, zs, wts = inversion_forward_process(ldm_stable, w0, etas=eta, prompt=prompt_src,
                                                    cfg_scale=cfg_scale_src, prog_bar=True,
                                                    num_inference_steps=args.num_diffusion_steps)
            # shape of wt (num_diffusion_steps+1,c,h,w) -> useless
            # length of zs: 100
            # length of wts(xts): 101
            # wt: xt, zs:[z0,z1,z2...],wts:[x0,x1,x2...]

        # iterate over decoder prompts
        for k in range(len(prompt_tar_list)):
            prompt_tar = prompt_tar_list[k]
            save_path = os.path.join(f'./results/', args.mode + xa_sa_string + str(time_stamp),
                                     image_path.split(sep='.')[0], 'src_' + prompt_src.replace(" ", "_")[:5],
                                     'dec_' + prompt_tar.replace(" ", "_")[:5])
            ...
            for cfg_scale_tar in cfg_scale_tar_list:
                for skip in skip_zs:
                    if args.mode == "our_inv":
                        ...
                        if src_tar_len_eq:
                            controller = AttentionReplace(prompts, args.num_diffusion_steps,
                            ...
                            controller = AttentionRefine(prompts, args.num_diffusion_steps, cross_replace_steps=args.xa,
                                                         self_replace_steps=args.sa, model=ldm_stable)

                        register_attention_control(ldm_stable, controller)
                        w0, _ = inversion_reverse_process(ldm_stable, xT=wts[args.num_diffusion_steps - skip], etas=eta,
                                                          prompts=prompts, cfg_scales=cfg_scale_list, prog_bar=True,
                        ...
                        if args.mode == "p2pddim":
                            if src_tar_len_eq:
                                controller = AttentionReplace(prompts, args.num_diffusion_steps, cross_replace_steps=.8,
                                                              self_replace_steps=0.4, model=ldm_stable)
                            # Should use Refine for target prompts with different number of tokens
                            else:
                                controller = AttentionRefine(prompts, args.num_diffusion_steps, cross_replace_steps=.8,
                                                             self_replace_steps=0.4, model=ldm_stable)
                        else:
                        ...
                        cfg_scale_list = [cfg_scale_src, cfg_scale_tar]
                        w0, latent = text2image_ldm_stable(ldm_stable, prompts, controller, args.num_diffusion_steps,
                                                           cfg_scale_list, None, wT)
                        w0 = w0[1:2]
                    else:
